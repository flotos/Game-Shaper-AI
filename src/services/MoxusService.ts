...
const saveMemory = () => {
  const success = safeLocalStorageSetItem(MOXUS_STRUCTURED_MEMORY_KEY, JSON.stringify(moxusStructuredMemory));
  if (!success) {
    console.error('[MoxusService] Failed to save Moxus memory even after automated cleanup attempts');
  }
};
